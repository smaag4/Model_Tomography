...
def get_activation(model, data):
    layer_output = Te.function([model.hiddenLayer.input], [model.hiddenLayer.output])
    return layer_output(data)

def show_scatterplot(points_transformed, targets, dataset, mode, m_type, X_ext, Y_predicted=None):
    if Y_predicted is None:
        palette = sns.color_palette('bright', 3)
        plt.figure(figsize=(10, 10))
        sns.scatterplot(points_transformed[:, 0], points_transformed[:, 1], hue=targets, legend='full', palette=palette)
        #plt.savefig('experiments/{}/plots/hiddenlayer_{}_{}_{}'.format(dataset))
        plt.savefig('experiments/{}/plots/hiddenlayer_TSNE_{}_{}_{}'.
                    format(dataset, mode, m_type, len(X_ext)))
    else:
...
def show_tsne(model_name, epochs, X, Y, dataset, mode, m_type, X_ext, Y_predicted=None, init=None):
    ...
    show_scatterplot(points_transformed, targets, dataset, mode, m_type, Y_predicted)
...
def build_model(nn_hdim, X, y, epsilon=1e-5, reg_lambda=0.0001, num_passes=1000,
    ...
    print ('... building model (hdim={}, len(X)={}, ' \
                         'batch_size={}, eps={}, eps_factor={})'.\
        format(nn_hdim, len(X), batch_size, epsilon, eps_factor), file=sys.stderr)
    ...
    l = 1
    g = 0
    new_batches = int(n_batches)
    while i < num_passes:
        i += 1
        for minibatch_index in xrange(new_batches):
        ...
        if i % print_epoch == 0:
            ...
            print ('Epoch %i: Cost: %f' % (i, curr_cost), file=sys.stderr)
...
class PerceptronExtractor(object):
    ...
    def extract(self, X_train, y_train, budget, steps=[],
        ...
        while budget > 0:
            ...
            print(self.calculate_loss(X_ext, y_ext_p, reg_lambda), file=sys.stderr)
            ...
            activation = get_activation(model, X_train)
            activation = np.array(activation)
            activation = activation[0, :, :]
            plt.figure()
            plt.scatter(activation[:, 0], activation[:, 1], s=40,
                        cmap=plt.cm.Spectral)
            plt.savefig('experiments/{}/plots/hiddenlayer_activation_{}_{}_{}'.
                        format(self.dataset,mode, m_type, len(X_ext)))

            transformed_points = show_tsne(model, 0, activation, y_train, self.dataset,  mode, m_type, X_ext)
            ...
            if budget > 0:
                ...
                samples = utils.line_search(X_local, Y_local, int(adaptive_budget/2),
                                            predict_func)
                X_random = utils.gen_query_set(X_ext.shape[1], int(random_budget))
